...
teno_order = 6
Avg = RoeAverage([0, 1])
LF = LLFTeno(teno_order, formulation='adaptive', averaging=Avg, sensor=sensor_array, store_sensor=store_sensor)
schemes = {}
schemes[LF.name] = LF
fns = 'u0 u1 u2'
...
trip_eqn = exp(-(x0 - 20)**2.0/ ( 0.425*2/0.1))  * 0.2* sin(0.15 * t) *exp( - ((z0 - 27.32)**2.0)/(2.0*(0.1*27.32)**2.0))
