...
def prepare_test_environment() -> List[str]:
    ...
    module. Test cases are copied into an _opensbli-build-workspace directory.
    ...
    test_cases = []
    if TEST_MODE is TestModes.SINGLE_TEST_CASE:
        app_name='(?<=/)'+app_build
        for test_case in APP_TEST_CASES:
            found = re.search(app_name,test_case)
            if found:
                test_cases.append(test_case)
    else:
        test_cases = APP_TEST_CASES


    workspace_dir = f"{SCRIPT_DIRECTORY}/_opensbli-build-workspace"
    # In case of not generating check if the workspace exists
    # If it does not force the workspace to be created and generation to be active
    if not GENERATE:
        found = os.path.isdir(workspace_dir)
        if found:
            _log(f"Workspace exists Generation Stage will be skip")
        else:
            _log(f"Workspace does not exists Generation Stage will be performed regardless")
            GENERATE=True
    ...
    for test_case in test_cases:
        ...
        _log(
            f"- Copying {os.path.basename(test_case)} to _opensbli-build-workspace/{os.path.basename(old_dir)}",
        ...
        if GENERATE:
            ...
            workspace_tests.append(f"{new_dir}/{os.path.basename(test_case)}")
...
def run_tests() -> None:
    ...
    for app_path in test_app_paths:
        ...
        #if GENERATE:
        _log("-" * 80)
        _log(f"Building: \033[1m{app_name}\033[0m")
...
if __name__ == "__main__":
    ...
    if app_build != "all":
        TEST_MODE = TestModes.SINGLE_TEST_CASE
