...
for block_number, block in enumerate(input_files):
    ...
    new_shape = tuple(reversed([shape[i]+ 2*nhalo[i] for i in range(ndim)]))
    ...
    print(y.shape)
    # exit()
    newx[nhalo[0]:new_shape[0] -nhalo[0], nhalo[1]:new_shape[1] -nhalo[1]] = np.transpose(x)
    newy[nhalo[0]:new_shape[0] -nhalo[0], nhalo[1]:new_shape[1] -nhalo[1]] = np.transpose(y)
    ...
    set_hdf5_metadata(dset, halos=halo, npoints=[shape[0], shape[1]], block=b)
