''' 
    chemical_kinetics: adds wdot source terms to opensbli framework along with all the other substitutions
                        potential update: need to find a way to add constants automatically

    author. 
    last update 26.06.2023

    syntax: 
            RE = add_rate_equations('gnoffo89', species=5, constants='park89', two_temperature=True) 
            
            constituent_eqns, substitutions = add_rate_equations(constituent_eqns, substitutions, 'gnoffo89', species=5, constants='park89', two_temperature=True) 

            potential options: park01 along
            constants: coded in dunnkang80 and park89 from Gnoffo et.al. 1989 paper

        example for two species (binary mixture)
            RE = add_rate_equations('gnoffo89', species=[N2, N], constants='park89', two_temperature=True)
            constituent_eqns, substitutions = add_rate_equations(constituent_eqns, substitutions, 'gnoffo89', species=['N2', 'N'], constants='park89', two_temperature=True) 


    one can extract variables in the code:
            wdotO2, wdotO, wdotN2, wdotN, wdotNO = RE.wdotO2, RE.wdotO, RE.wdotN2, RE.wdotN, RE.wdotNO
...
class gnoffo89(object):
    '''
      gnoffo89 paper - (Gnoffo et al. pg 35, table III, r=1,2,3,4,5,6,8,9,10) 
        set of equations:

        R1 - O2 + M  ↔ 2O + M (M = N, O); 					alphar1, betar1 
        R2 - O2 + M  ↔ 2O + M (M = N2, O2, NO)				alphar2, betar2 

        R3 - N2 + N  ↔ 2N + N (3N)							alphar3, betar3
        R4 - N2 + O  ↔ 2N + O								alphar4, betar4
        R5 - N2 + M  ↔ 2N + M (M = N2, O2)					alphar5, betar5
        R6 - N2 + NO ↔ 2N + NO 								alphar6, betar6

        R7 - NO + M  ↔ N + O + M (M = N, O, N2, O2, NO)		alphar7, betar7
        R8 - NO + O  ↔ O2 + N								alphar8, betar8

        R9 - N2 + O  ↔ NO + N								alphar9, betar9

     - potentially change R9 to be R7 or R5, closer to N2 reactions. 
      stoichiometric coefficients are self explanatory, but are included for posterity 

    Rf_s and Rb_s equations are eqs.42 and 43 from gnoffo89 paper. 
    '''
    ...
    def rate_equations(self, species):
        ...
        if species == 5: 
            wdotO2 = 'Eq(wdotO2 , MO2 * ((0.0 - 1.0)*(Rf1-Rb1) + (1.0 - 2.0)*(Rf2-Rb2) + (1.0 - 0.0)*(Rf8-Rb8)))'
            wdotO  = 'Eq(wdotO  , MO  * ((3.0 - 1.0)*(Rf1-Rb1) + (2.0 - 0.0)*(Rf2-Rb2) + (2.0 - 1.0)*(Rf7-Rb7) + (0.0 - 1.0)*(Rf8-Rb8) + (0.0 - 1.0)*(Rf9-Rb9)))'
            wdotN2 = 'Eq(wdotN2 , MN2 * ((0.0 - 1.0)*(Rf3-Rb3) + (0.0 - 1.0)*(Rf4-Rb4) + (1.0 - 2.0)*(Rf5-Rb5) + (0.0 - 1.0)*(Rf6-Rb6)))'
            wdotN  = 'Eq(wdotN  , MN  * ((3.0 - 1.0)*(Rf3-Rb3) + (2.0 - 0.0)*(Rf4-Rb4) + (2.0 - 0.0)*(Rf5-Rb5) + (2.0 - 0.0)*(Rf6-Rb6) + (1.0 - 0.0)*(Rf7-Rb7) + (2.0 - 1.0)*(Rf8-Rb8) + (1.0 - 0.0)*(Rf9-Rb9)))'
            wdotNO = 'Eq(wdotNO , MNO * ((0.0 - 1.0)*(Rf7-Rb7) + (0.0 - 1.0)*(Rf8-Rb8) + (1.0 - 0.0)*(Rf9-Rb9)))'
    ...
    def reaction_dependencies(self):
        substitutions = []
        # forward and backward rate equations, Rfr, and Rbr respectively forming park89 framework
        Rf1 = 'Eq(Rf1, 1000 * kf1         * (0.001*   rhoO/MO)**(2.0) * (0.001* rhoN/MN + 0.001* rhoO/MO)**(1.0) )'
        Rb1 = 'Eq(Rb1, 1000 * (kf1 / Kc1) * (0.001*   rhoO/MO)**(2.0) * (0.001* rhoN/MN + 0.001* rhoO/MO)**(1.0))'

        Rf2 = 'Eq(Rf2, 1000 * kf1         * (0.001* rhoO2/MO2)**(1.0) * (0.001* rhoN2/MN2 + 0.001* rhoO2/MO + 0.001* rhoNO/MNO)**(1.0))'
        Rb2 = 'Eq(Rb2, 1000 * (kf6 / Kc6) * (0.001*   rhoO/MO)**(2.0) * (0.001* rhoN2/MN2 + 0.001* rhoO2/MO + 0.001* rhoNO/MNO)**(1.0))'

        Rf3 = 'Eq(Rf3, 1000 * kf3         * (0.001* rhoN2/MN2)**(1.0) * (0.001* rhoN/MN)**(1.0))'
        Rb3 = 'Eq(Rb3, 1000 * (kf3 / Kc3) * (0.001*   rhoN/MN)**(3.0))'

        Rf4 = 'Eq(Rf4, 1000 * kf4         * (0.001* rhoN2/MN2)**(1.0) * (0.001* rhoN/MO)**(1.0))'
        Rb4 = 'Eq(Rb4, 1000 * (kf4 / Kc4) * (0.001*   rhoN/MN)**(2.0) * (0.001* rhoN/MO)**(1.0))'
        
        Rf5 = 'Eq(Rf5, 1000 * kf5         * (0.001* rhoN2/MN2)**(1.0) * (0.001* rhoN2/MN2 + 0.001* rhoO2/MO)**(1.0))'
        Rb5 = 'Eq(Rb5, 1000 * (kf5 / Kc5) * (0.001*   rhoN/MN)**(2.0) * (0.001* rhoN2/MN2 + 0.001* rhoO2/MO)**(1.0))'

        Rf6 = 'Eq(Rf6, 1000 * kf6         * (0.001* rhoN2/MN2)**(1.0) * (0.001* rhoNO/MNO)**(1.0))'
        Rb6 = 'Eq(Rb6, 1000 * (kf6 / Kc6) * (0.001*   rhoN/MN)**(2.0) * (0.001* rhoNO/MNO)**(1.0))'

        Rf7 = 'Eq(Rf7, 1000 * kf7         * (0.001* rhoNO/MNO)**(1.0) * (0.001* rhoN/MN + 0.001* rhoO/MO + 0.001* rhoN2/MN2 + 0.001* rhoN/MN + 0.001* rhoNO/MNO )**(1.0))'
        Rb7 = 'Eq(Rb7, 1000 * (kf7 / Kc7) * (0.001*   rhoN/MN)**(1.0) * (0.001* rhoO/MO)**(1.0) * (0.001* rhoN/MN + 0.001* rhoO/MO + 0.001* rhoN2/MN2 + 0.001* rhoN/MN + 0.001* rhoNO/MNO )**(1.0))'

        Rf8 = 'Eq(Rf8, 1000 * kf8         * (0.001* rhoNO/MNO)**(1.0) * (0.001* rhoN/MO)**(1.0))'
        Rb8 = 'Eq(Rb8, 1000 * (kf8 / Kc8) * (0.001* rhoO2/MO2)**(1.0) * (0.001* rhoN/MN)**(1.0))'

        Rf9 = 'Eq(Rf9, 1000 * kf9         * (0.001* rhoN2/MN2)**(1.0) * (0.001* rhoN/MO)**(1.0))'
        Rb9 = 'Eq(Rb9, 1000 * (kf9 / Kc9) * (0.001* rhoNO/MNO)**(1.0) * (0.001* rhoN/MN)**(1.0))'

        substitutions += [Rf1,Rb1, Rf2,Rb2, Rf3,Rb3, Rf4,Rb4, Rf5,Rb5, Rf6,Rb6, Rf7,Rb7, Rf8,Rb8, Rf9,Rb9]

        # forward reaction rates, kfr, backward are already in the code since kbr = (kfr/ Kcr)
        kf1 = 'Eq(kf1, Cf1 * Tq**(nf1) * exp( - thetaf1/ Tq))' # here, Ef1/k values are provided in table IV, hence, theta1= Ef1/k
        ...
        kf8 = 'Eq(kf8, Cf8 * Tq**(nf8) * exp( - thetaf8/ Tq))'
        kf9 = 'Eq(kf9, Cf9 * Tq**(nf9) * exp( - thetaf9/ Tq))'

        substitutions += [kf1, kf2, kf3, kf4, kf5, kf6, kf7, kf8, kf9]

        # equilibrium constant for reaction, r, employed by park89 curvefit
        Kc1 = 'Eq(Kc1, exp(B11 + B12*ln(Z) + B13*Z + B14*Z**2.0 + B15*Z**3.0))'
        Kc2 = 'Eq(Kc2, exp(B21 + B22*ln(Z) + B23*Z + B24*Z**2.0 + B25*Z**3.0))'
        Kc3 = 'Eq(Kc3, exp(B31 + B32*ln(Z) + B33*Z + B34*Z**2.0 + B35*Z**3.0))'
        Kc4 = 'Eq(Kc4, exp(B41 + B42*ln(Z) + B43*Z + B44*Z**2.0 + B45*Z**3.0))'
        Kc5 = 'Eq(Kc5, exp(B51 + B52*ln(Z) + B53*Z + B54*Z**2.0 + B55*Z**3.0))'
        Kc6 = 'Eq(Kc6, exp(B61 + B62*ln(Z) + B63*Z + B64*Z**2.0 + B65*Z**3.0))'
        Kc7 = 'Eq(Kc7, exp(B71 + B72*ln(Z) + B73*Z + B74*Z**2.0 + B75*Z**3.0))'
        Kc8 = 'Eq(Kc8, exp(B81 + B82*ln(Z) + B83*Z + B84*Z**2.0 + B85*Z**3.0))'
        Kc9 = 'Eq(Kc9, exp(B91 + B92*ln(Z) + B93*Z + B94*Z**2.0 + B95*Z**3.0))'

        substitutions += [Kc1, Kc2, Kc3, Kc4, Kc5, Kc6, Kc7, Kc8, Kc9]
    ...
    def generate_constants(self):
        ''' constants extracted from table IV, dunnkang set of constants
            Gnoffo et.al 1989 paper
        '''
        Cf1, nf1, theta1, B11, B12, B13, B14, B15 = ConstantObject('Cf1'), ConstantObject('nf1'), ConstantObject('theta1'), ConstantObject('B11'), ConstantObject('B12'), ConstantObject('B13'), ConstantObject('B14'), ConstantObject('B15')
        Cf2, nf2, theta2, B21, B22, B23, B24, B25 = ConstantObject('Cf2'), ConstantObject('nf2'), ConstantObject('theta2'), ConstantObject('B21'), ConstantObject('B22'), ConstantObject('B23'), ConstantObject('B24'), ConstantObject('B25')
        Cf3, nf3, theta3, B31, B32, B33, B34, B35 = ConstantObject('Cf3'), ConstantObject('nf3'), ConstantObject('theta3'), ConstantObject('B31'), ConstantObject('B32'), ConstantObject('B33'), ConstantObject('B34'), ConstantObject('B35')
        Cf4, nf4, theta4, B41, B42, B43, B44, B45 = ConstantObject('Cf4'), ConstantObject('nf4'), ConstantObject('theta4'), ConstantObject('B41'), ConstantObject('B42'), ConstantObject('B43'), ConstantObject('B44'), ConstantObject('B45')
        Cf5, nf5, theta5, B51, B52, B53, B54, B55 = ConstantObject('Cf5'), ConstantObject('nf5'), ConstantObject('theta5'), ConstantObject('B51'), ConstantObject('B52'), ConstantObject('B53'), ConstantObject('B54'), ConstantObject('B55')
        Cf6, nf6, theta6, B61, B62, B63, B64, B65 = ConstantObject('Cf6'), ConstantObject('nf6'), ConstantObject('theta6'), ConstantObject('B61'), ConstantObject('B62'), ConstantObject('B63'), ConstantObject('B64'), ConstantObject('B65')
        Cf7, nf7, theta7, B71, B72, B73, B74, B75 = ConstantObject('Cf7'), ConstantObject('nf7'), ConstantObject('theta7'), ConstantObject('B71'), ConstantObject('B72'), ConstantObject('B73'), ConstantObject('B74'), ConstantObject('B75')
        Cf8, nf8, theta8, B81, B82, B83, B84, B85 = ConstantObject('Cf8'), ConstantObject('nf8'), ConstantObject('theta8'), ConstantObject('B81'), ConstantObject('B82'), ConstantObject('B83'), ConstantObject('B84'), ConstantObject('B85')
        Cf9, nf9, theta9, B91, B92, B93, B94, B95 = ConstantObject('Cf9'), ConstantObject('nf9'), ConstantObject('theta9'), ConstantObject('B91'), ConstantObject('B92'), ConstantObject('B93'), ConstantObject('B94'), ConstantObject('B95')

        input_constants  = []
        input_constants += ['Cf1', 'nf1', 'theta1', 'B11', 'B12', 'B13', 'B14', 'B15']
        input_constants += ['Cf2', 'nf2', 'theta2', 'B21', 'B22', 'B23', 'B24', 'B25']
        input_constants += ['Cf3', 'nf3', 'theta3', 'B31', 'B32', 'B33', 'B34', 'B35']
        input_constants += ['Cf4', 'nf4', 'theta4', 'B41', 'B42', 'B43', 'B44', 'B45']
        input_constants += ['Cf5', 'nf5', 'theta5', 'B51', 'B52', 'B53', 'B54', 'B55']
        input_constants += ['Cf6', 'nf6', 'theta6', 'B61', 'B62', 'B63', 'B64', 'B65']
        input_constants += ['Cf7', 'nf7', 'theta7', 'B71', 'B72', 'B73', 'B74', 'B75']
        input_constants += ['Cf8', 'nf8', 'theta8', 'B81', 'B82', 'B83', 'B84', 'B85']
        input_constants += ['Cf9', 'nf9', 'theta9', 'B91', 'B92', 'B93', 'B94', 'B95']


        # kinetic model of park, park89 constants 
        Cf1.value, nf1.value, theta1.value, B11.value, B12.value, B13.value, B14.value, B15.value = 2.000e23, -2.00, 5.975e4,  2.855,  0.988, -6.181, -0.023, -0.001
        Cf2.value, nf2.value, theta2.value, B21.value, B22.value, B23.value, B24.value, B25.value = 9.680e22, -2.00, 5.975e4,  2.855,  0.988, -6.181, -0.023, -0.001
        Cf3.value, nf3.value, theta3.value, B31.value, B32.value, B33.value, B34.value, B35.value = 1.600e22, -1.60, 1.132e5,  1.858, -1.325, -9.856, -0.174,  0.008
        Cf4.value, nf4.value, theta4.value, B41.value, B42.value, B43.value, B44.value, B45.value = 4.980e22, -1.60, 1.132e5,  1.858, -1.325, -9.856, -0.174,  0.008
        Cf5.value, nf5.value, theta5.value, B51.value, B52.value, B53.value, B54.value, B55.value = 3.700e21, -1.60, 1.132e5,  1.858, -1.325, -9.856, -0.174,  0.008
        Cf6.value, nf6.value, theta6.value, B61.value, B62.value, B63.value, B64.value, B65.value = 4.980e21, -1.60, 1.132e5,  1.858, -1.325, -9.856, -0.174,  0.008
        Cf7.value, nf7.value, theta7.value, B71.value, B72.value, B73.value, B74.value, B75.value = 7.950e23, -2.00, 7.550e4,  0.792, -0.492, -6.761, -0.091,  0.004
        Cf8.value, nf8.value, theta8.value, B81.value, B82.value, B83.value, B84.value, B85.value = 8.370e12,  0.00, 1.945e4, -2.063, -1.480, -0.580, -0.114,  0.005
...
class add_rate_equations(object):
    ...
    def __init__(self, constituent_eqns, substitutions, constants, simulation_type='noneq', equation_type='gnoffo89', species=5, reaction_constants='park89', two_temperature=False):
        self.contituent_eqns = constituent_eqns
        self.substitutions   = substitutions
        # print(constants)
        self.constants = constants

        if simulation_type == 'frozen':
            print('generating source terms for frozen flow')
            wdotO2 = 'Eq(wdotO2 , 0.0)'
            wdotO  = 'Eq(wdotO  , 0.0)'
            wdotN2 = 'Eq(wdotN2 , 0.0)'
            wdotN  = 'Eq(wdotN  , 0.0)'
            wdotNO = 'Eq(wdotNO , 0.0)'
            self.contituent_eqns += [wdotO2, wdotO, wdotN2, wdotN, wdotNO]

        else:
            print('generating source terms for equilibrium/nonequilibrum flow')
        
            if equation_type == 'gnoffo89':
                print('adding rate equation source terms of gnoffo89')
                gnoffo89(self.contituent_eqns, self.substitutions, self.constants, species, reaction_constants, two_temperature)
                # raise NotImplementedError('only frozen flow wdot terms are implemented')
            elif equation_type == 'park01':
                # raise NotImplementedError('only gnoffo89 is currently implemented, updates will include park01')
