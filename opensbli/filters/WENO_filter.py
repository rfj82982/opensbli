...
class WENOFilter(NonSimulationEquations):
    ...
    def evaluate_shock_sensor(self, block):
        ...
        DT.value = 0.5
        CTD.add_constant(DT)
        evaluation_condition = [ExprCondPair(1, kappa_evaluation > DT)]
        # # No wall or interface, default condition is the sensor is not turned off
        evaluation_condition += [ExprCondPair(0, True)]
        sensor_evaluations += [OpenSBLIEq(kappa, Piecewise(*evaluation_condition))]
    ...
    def main(self, scheme_order, block):
        ...
        if self.flux_type == 'LLF' or self.flux_type == 'GLF':
            ...
            WS = HLLCWeno(scheme_order, formulation='JS', flux_type=self.flux_type, averaging=RoeAverage([0, 1]), shock_filter=True, conservative=block.conservative)
        else:
