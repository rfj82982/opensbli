...
from sympy import Rational, S, Float, sqrt, Matrix, pprint, exp, Mul, Pow
from opensbli.schemes.spatial.weno import ShockCapturing
from opensbli.core.grid import GridVariable
# from sympy import sqrt, Matrix, pprint, exp, Mul, Pow


class chemicalForcingStripBC(ModifyCentralDerivative, BoundaryConditionBase):
    ...
    def __init__(self, direction, side, wall_normal_velocity, scheme=None, plane=True, catalytic_condition=False):
        BoundaryConditionBase.__init__(self, direction, side, plane)
        self.bc_name = 'chemicalForcingStripWall'
        # self.equations = equations
        self.wall_normal_velocity = wall_normal_velocity
        self.catalytic_condition = catalytic_condition
        if not scheme:
    ...
    def apply(self, arrays, block):
        ...
        if self.catalytic_condition == False:
            # Define constants
            ...
            coeffs, grid_indices = self.deriv_coeffs[side], self.deriv_indices[side]
            # Evaluate yNO values at the wall and 5 points above/below it
            cN2_values = [GridVariable('cN2%d' % i) for i in range(len(grid_indices))]
            
            cN2_evaluations = [OpenSBLIEq(cN2_values[i], increment_dataset(rN2 / (rN2 + rN + rO2 + rO + rNO), self.direction, i*to_side_factor))
                                    for i in range(1, len(cN2_values))]

            wall_eqns += cN2_evaluations
            ...
            coeffs, grid_indices = self.deriv_coeffs[side], self.deriv_indices[side]
            # Evaluate yNO values at the wall and 5 points above/below it
            cO2_values = [GridVariable('cO2%d' % i) for i in range(len(grid_indices))]
            
            cO2_evaluations = [OpenSBLIEq(cO2_values[i], increment_dataset(rO2 / (rN2 + rN + rO2 + rO + rNO), self.direction, i*to_side_factor))
                                    for i in range(1, len(cO2_values))]

            wall_eqns += cO2_evaluations
            ...
            coeffs, grid_indices = self.deriv_coeffs[side], self.deriv_indices[side]
            # Evaluate yNO values at the wall and 5 points above/below it
            cNO_values = [GridVariable('rhoNO%d' % i) for i in range(len(grid_indices))]
            ...
            wall_eqns += [OpenSBLIEq(energy_store, evib_store + hf0*(rO*(dhO/MO)+rN*(dhN/MN)+rNO*(dhNO/MNO)) +  Mul(Twall, Rhat*((3.0/2.0)*((rO/MO)+(rN/MN))+(5.0/2.0)*((rO2/MO2)+(rN2/MN2)+(rNO/MNO)))) )]
            # wall_eqns += [OpenSBLIEq(energy_store, evib_store + 0.0*(rO*(dhO/MO)+rN*(dhN/MN)+rNO*(dhNO/MNO)) +  Mul(Twall, Rhat*((3.0/2.0)*((rO/MO)+(rN/MN))+(5.0/2.0)*((rO2/MO2)+(rN2/MN2)+(rNO/MNO)))) )]
