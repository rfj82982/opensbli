...
metriceq.generate_transformations(
    ndim, coordinate_symbol, [(True, True), (True, True)], 2
)
...
for i, base in enumerate(base_eqns):
    base_eqns[i] = einstein.expand(
        base, ndim, coordinate_symbol, substitutions, constants_eqns
    )
    if base == momentum:
...
for i, CR in enumerate(constituent_eqns):
    constituent_eqns[i] = einstein.expand(
        CR, ndim, coordinate_symbol, substitutions, constants_eqns
    )
...
x0 = Eq(
    DataObject("x0"),
    i * dx + Ax * sin(2.0 * pi * omega) * sin(Sx * pi * j * dy / Ly),
    evaluate=False,
)
x1 = Eq(
    DataObject("x1"),
    j * dy + Ay * sin(2.0 * pi * omega) * sin(Sy * pi * i * dx / Lx),
    evaluate=False,
)
